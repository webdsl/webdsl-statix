  ...
  define page root(){
    ...
    init{
        d := DateThingy{date := Date("01/02/2003") datetime := now() time := Time("12:34")}; //DateTime("01/02/2003 12:34")
          ...
          var i2 := Item{
              name := "Diet Coke"
              description := "Amount (% RDA Ingredients) Calories 0 Fat 0g (0%) Sodium 40mg (2%) Carbohydrates 0g (0%) Sugar 0g Protein 0g (0%) Diet Coke (TM) - Ingredients Carbonated Water    High Fructose Corn Syrup Caramel Color Phosphoric Acid Natural Flavors Caffeine Aspartame (NutraSweet) Potassium Benzoate Citric acid. TheEnd."
            };
          var i3 := Item{
              name := "Car"
  ...
  test AdvancedSearch {
    ...
    while (runTwice < 2){
        ...
        if (runTwice == 1){
          ...
          link.click();
          sleep(1000); //will probably fix build farm issue, where it tries to get page source before finishing loading the page
          pagesource := d.getPageSource();
